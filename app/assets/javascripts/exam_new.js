...
$(function(){
	...
	function createTextArea(selectId, questionId){
        ...
        // 穴埋め問題には、注意書きを書く。
        if(selectId == 4){  
          $("select[name = \"question["+questionId+"][kind]\"]").after("<br /><br /><div id = \"body"+questionId+"\">問題にする部分を、#{}でくくってください。<br />Ex.りんごは英語で、#{apple}。<br />問題文<br /><textarea cols=\"40\" name=\"question["+questionId+"][body]\" rows=\"20\"></textarea><br></div>");
        }else{
	...
	function addEvent(questionId){
		...
		var tag = "<div id = \"form" + nextQuestionId + "\"><br />" + (nextQuestionId + 1) + "問目<br />問題形式 : <select name = \"question[" + nextQuestionId + "][kind]\"><option value=\"0\">選択してください</option><option value=\"1\">N択問題</option><option value=\"2\">○×問題</option><option value=\"3\">一問一答</option></select><br /></div>";
	...
	function createForm(questionId){
		...
		$("select[name = \"question["+ questionId + "][kind]\"]").bind("change",function(){
			...
			if(selectId == 1){
				...
				init(questionId);
				createTextArea(selectId,questionId);
				$("#body" + questionId).after("<div id = \"TrueOrFalse" + questionId + "\">○か×か。<select name = \"question[" + questionId + "][choices][0][right]\" ><option value = \"1\">○</option><option value = \"0\">×</option></select></div><input name=\"commit\" class=\"btn\" type=\"submit\" value=\"送信\" /><input type = \"button\" class=\"btn\" value = \"問題を追加\" id = \"add\" />");
				addEvent(questionId);
			}else if(selectId == 3){
				init(questionId);
				createTextArea(selectId,questionId);
				$("#body" + questionId).after("<div id = \"right" + questionId + "\">正解は、<input type = \"text\" class=\"span3\" size = \"16\" maxlinght = \"16\" name = \"question[" + questionId + "][choices][0][choice_text]\" /></div><input name = \"commit\" type=\"submit\" value=\"送信\" class=\"btn\"/><input type = \"button\" class=\"btn\" value = \"問題を追加\" id = \"add\" />");
