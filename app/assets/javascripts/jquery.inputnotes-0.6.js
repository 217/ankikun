...
(function($) {
        ...
        $.fn.inputNotes=function(notes) {
                ...
                var d = "#"+area.attr('id')+"_error";      // span id
                $(c).hide();
        
                var notebox = $('#'+area.attr('id')+'_inputnotes');
                notebox.css({ 'float' : 'right', 'width': area.outerWidth(), 'display' : 'inline' });
                ...
                function showNotes(){
                        ...
                        for(var item in notes){
                                ...
                                if ((!n.inversedBehavior && content.match(n.pattern)) || (n.inversedBehavior && !content.match(n.pattern))){
                                        if ($(itemdiv).size() == 0){
                                                $(c).append('<'+config.noteTag+' id="note_'+item+'" class="'+n.type+'" style="float: left;">'+n.text+'</'+config.noteTag+'>');
                                                if ($(c).is(':hidden')){
                                                        $(c).show();
                                                }
